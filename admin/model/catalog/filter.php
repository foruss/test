...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		$r = $this->db->query("DESCRIBE " . DB_PREFIX . "filter_group `manufacturer`");
		if ($r->num_rows == 0) {
			$msql  = "ALTER TABLE `" . DB_PREFIX . "filter_group` ADD `manufacturer`  TINYINT(1) NOT NULL AFTER `sort_order`";
			$query = $this->db->query($msql);
		}

		$this->db->query("INSERT INTO `" . DB_PREFIX . "filter_group` SET sort_order = '" . (int)$data['sort_order'] . "' , manufacturer = '" . (int)$data['manufacturer'] . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "filter_group` SET sort_order = '" . (int)$data['sort_order'] . "' , manufacturer = '" . (int)$data['manufacturer'] . "' WHERE filter_group_id = '" . (int)$filter_group_id . "'");
	...
	public function getFilterGroupByName($filter_group_name) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "filter_group` fg LEFT JOIN " . DB_PREFIX . "filter_group_description fgd ON (fg.filter_group_id = fgd.filter_group_id) WHERE fgd.name = '" . $filter_group_name . "' AND fgd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
