...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	protected function getForm() {
		...
		$this->data['entry_product_hwy'] = $this->language->get('entry_product_hwy');
		$this->data['entry_product_city'] = $this->language->get('entry_product_city');
		...
		$this->data['tab_newoption'] = $this->language->get('tab_newoption');
		...
		if (isset($this->request->post['product_hwy'])) {
      		$this->data['product_hwy'] = $this->request->post['product_hwy'];
    	} elseif (!empty($product_info)) {
			$this->data['product_hwy'] = $product_info['product_hwy'];
		} else {
      		$this->data['product_hwy'] = '';
    	}
		...
		if (isset($this->request->post['product_city'])) {
      		$this->data['product_city'] = $this->request->post['product_city'];
    	} elseif (!empty($product_info)) {
			$this->data['product_city'] = $product_info['product_city'];
		} else {
      		$this->data['product_city'] = '';
    	}
		...
		$this->data['filters'] = $this->model_catalog_filter->getFilters(0);
		...
		foreach ($filters as $filter_id) {
			...
			if ($filter_info) {
			if ($filter_info['filter_group_id'] == 6) {$this->data['product_interiors_id'] = $filter_info['filter_id']; continue;}
			if ($filter_info['filter_group_id'] == 2) {$this->data['product_transmissions_id'] = $filter_info['filter_id']; continue;}
			if ($filter_info['filter_group_id'] == 5) {$this->data['product_colors_id'] = $filter_info['filter_id']; continue;}
			if ($filter_info['filter_group_id'] == 4) {$this->data['product_fuels_id'] = $filter_info['filter_id']; continue;}
			if ($filter_info['filter_group_id'] == 9) {$this->data['product_drivetypes_id'] = $filter_info['filter_id']; continue;}
			$this->data['product_model_id'] = $filter_info['filter_id']; continue;
		...
		$transmissions = $this->model_catalog_filter->getFilterDescriptions(2);
		$this->data['product_transmissions'] = array();
		foreach ($transmissions as $transmission) {	
			$this->data['product_transmissions'][] = array(
					'filter_id' => $transmission['filter_id'],
					'name'      => $transmission['filter_description'][1]['name']
				);	
		}
		
		
		
		$interiors = $this->model_catalog_filter->getFilterDescriptions(6);
		$this->data['product_interiors'] = array();
		foreach ($interiors as $interior) {	
			$this->data['product_interiors'][] = array(
					'filter_id' => $interior['filter_id'],
					'name'      => $interior['filter_description'][1]['name']
				);	
		}
		
		$colors = $this->model_catalog_filter->getFilterDescriptions(5);
		$this->data['product_colors'] = array();
		foreach ($colors as $color) {	
			$this->data['product_colors'][] = array(
					'filter_id' => $color['filter_id'],
					'name'      => $color['filter_description'][1]['name']
				);	
		}
		
		$fuels = $this->model_catalog_filter->getFilterDescriptions(4);
		$this->data['product_fuels'] = array();
		foreach ($fuels as $fuel) {	
			$this->data['product_fuels'][] = array(
					'filter_id' => $fuel['filter_id'],
					'name'      => $fuel['filter_description'][1]['name']
				);	
		}
		
		$drivetypes = $this->model_catalog_filter->getFilterDescriptions(9);
		$this->data['product_drivetypes'] = array();
		foreach ($drivetypes as $drivetype) {	
			$this->data['product_drivetypes'][] = array(
					'filter_id' => $drivetype['filter_id'],
					'name'      => $drivetype['filter_description'][1]['name']
				);	
		}
		...
		if (isset($this->request->post['product_newoption'])) {
			$product_newoptions = $this->request->post['product_newoption'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_newoptions = $this->model_catalog_product->getProductNewoptions($this->request->get['product_id']);			
		} else {
			$product_newoptions = array();
		}			

		$product_newalloptions = $this->model_catalog_product->getProductNewalloptions();

		$this->data['product_newoptions'] = array();
			
		foreach ($product_newalloptions as $newoption) {
			$value=0;
			foreach ($product_newoptions as $product_newoption) {
					if ($newoption['newoption_id']==$product_newoption['newoption_id']) { $value=$product_newoption['value']; }
				}
				
				$this->data['product_newoptions'][] = array(								
					'newoption_id'         => $newoption['newoption_id'],
					'name'                 => $newoption['name'],
					'value'                => $value
				);				
		...
		$this->data['image_path'] = HTTP_CATALOG_IMAGE;
		// Downloads
	...
	public function checkmodel() {
		...
		if (isset($this->request->get['manufacturer_id'])) {
			...
			$fgi = $this->model_catalog_filter->getFilterGroupByName($manufacturer['name']);
			$results = $this->model_catalog_filter->getFilterDescriptions($fgi['filter_group_id']);
			asort($results);
			foreach ($results as $result) {
				$option_data = array();
				...
				$json[] = array(
					...
					'name'       => $result['filter_description'][1]['name']	
				);	
			}
		}
