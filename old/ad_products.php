...
$kiekis = $otas['sandelyje'];
$info = stripslashes($otas['info']);
$vieta = stripslashes($otas['vieta']);
if ($ktype == '1') { $kt = '€'; } else if ($ktype == '2') { $kt='$'; } 

$ffoto=mysql_query("Select * from `foto` where `sid`='$id' and `type`='9' order by `id` asc LIMIT 0,1");
...
$gff=mysql_query("Select * from `foto` where `sid`='$id' and `type`='9' order by `id` asc");
$i = 1;
					...
					<div class="u_block" style="padding: 0">
						<div class="u_block-cont" style="padding: 0">
							<table cellpadding="0" cellspacing="0" border="0">
								...
								<tr class="gr">
									<td style="width:140px;"><strong>Марка:</strong>:</td>
								  ...
								  <td><div><?php echo $bukle; ?></div></td>
								</tr>	<tr>
									<td><strong>Количество:</strong></td>
								  <td><div><?php echo $kiekis; ?></div></td>
								</tr>
							</table>		
						</div>
...
<div class="auto_info" style="margin: 0; padding: 15px 0 0 0">
					...
					<ul class="a_i_menu" style="margin: 0; padding: 0">
                        <li class="act" id="d1"><a href="#"><span>Информация</span></a></li>
                 		<li id="d5"><a href="#"><span onclick="window.open('https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=automixs@yahoo.com&item_name=Honda-Accord-2008&item_number=9685&amount=9700&currency_code=USD')">Оставить депозит</span></a></li>
					</ul>
...
<?php  echo $info; ?>
				...
				<?php 
				$id = $otas['id'];
				$updazz=mysql_query("Update `sk` set `viewed`=viewed+1 where `id`='$id'");
}
