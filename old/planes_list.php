...
$sk = mysql_num_rows(mysql_query("Select * from `plane`"));
$viso = intval($sk/$kiek);
if($sk%$kiek) $viso++;
if ($nuo > $viso) $nuo = 1;
$recordStart = ($nuo*$kiek)-$kiek;

$getitz=mysql_query("Select * from `plane` order by `id` desc  LIMIT $recordStart, $kiek") or die(mysql_error());

while($roz=mysql_fetch_array($getitz)) {
$sid = $roz['id'];

$get_foto=mysql_query("Select * from `foto` where `sid`='$sid' and `type`='4' order by `id` asc");
...
echo('<div style="width: 785px; height: 118px; background: #F7F7F7; margin: 6px 0 0 0"><a title="'.$marke.' '.$modelis.', '.$metai.'" href="plane/show/'.$sid.'/"><div style="float: left; width: 174px"><img alt="'.$marke.' '.$modelis.', '.$metai.'" src="phpThumb.php?src='.$foto.'&w=122&h=98" onmouseover="style.border=\'1px solid #990000\'" onmouseout="style.border=\'1px solid #999999\'" width="122" height="98" style="border: 1px solid #999999; margin: 9px 7px;"></a></div>
<div style="float: left; width: 442px; margin: 9px 0 0 0"><div style="margin: 0 0 10px 0"><a title="'.$marke.' '.$modelis.', '.$metai.'" class="sktop" href="plane/show/'.$sid.'/">'.$marke.' '.$modelis.', '.$metai.'</a></div>
...
<div style="float: left; width: 90px; text-align: right; height: 18px">Винт:</div><div style="color: #990000; float: left; width: 345px; margin: 0 0 0 5px; height: 18px">'.$propeleris.'</div>
<div style="float: left; width: 90px; text-align: right; height: 18px;">Макс. скорость:</div><div style="color: #990000; float: left; width: 345px; margin: 0 0 0 5px; height: 18px">'.$maxgreitis.'</div>
