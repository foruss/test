...
else
{
	...
	$mynewurl="index.php?mode=searchresults&trf=$trf&tracktype=$tracktype&make=$make&model=$model&maxprice=$maxprice&rn=$_GET['rn']";
...
$url='http://www.cars.com/go/search/search.jsp?tracktype='.$tracktype.'&srv=adlocator&aff=national&act=search&searchType=21&szc=10001&flt=zr%2Cn_ms%2Cused&fs=&so=desc&sb=vp&trf='.$trf.'&mknm='
	...
	$true_url=trim(str_replace(' ','%20',$location[1]));
	if(isset($_GET['rn']))
	$html_result = http_load($true_url.'&rn='.($_GET['rn']*50-50),null,null,null,null);
	else
	$html_result = http_load($true_url,null,null,null,null);
...
$pages[] = array('num' => $i, 'link' => "index.php?mode=searchresults&trf=$trf&tracktype=$tracktype&make=$make&model=$model&maxprice=$maxprice&rn=$i");
$dots = false;
} else if (!$dots) {
$pages[] = array('num' => '...');
$dots = true;
