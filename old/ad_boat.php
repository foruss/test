...
$plotis = stripslashes($otas['plotis']);
$grimzle = stripslashes($otas['grimzle']);
$vietos = stripslashes($otas['vietos']);
$ksanaud = stripslashes($otas['ksanaud']);
...
$ffoto=mysql_query("Select * from `foto` where `sid`='$id' and `type`='3' order by `id` asc LIMIT 0,1");
...
$gff=mysql_query("Select * from `foto` where `sid`='$id' and `type`='3' order by `id` asc");
$i = 1;
						...
						<div class="u_block-cont" style="padding: 0">
							<table cellpadding="0" cellspacing="0" border="0">
								...
								<tr>
									...
									<td><div><?php echo $plotis; ?></div></td>
								</tr>
								<tr class="gr">
									<td><strong>Осадка:</strong></td>
									<td><div><?php echo $grimzle; ?></div></td>
								</tr>
								<tr>
									<td><strong>Мест:</strong></td>
									<td><div><?php echo $vietos; ?></div></td>
								</tr>
							<tr class="gr">
									<td><strong>Потребление топлива:</strong></td>
								  <td><div><?php echo $ksanaud; ?></div></td>
								</tr>
									<tr>
								...
								<tr class="gr">
								  ...
								  <td><div><?php echo $kt.' '.$kaina; ?></div></td>
								</tr>
									<tr>
...
$buk = '<div style="padding: 2px; margin: 0 0 0 2px; height: 20; background: #FF9900; text-align: center; color: #FFFFFF">Плохое</div>';
}
	...
	?>
				...
				<?php 
				$id = $otas['id'];
				$updazz=mysql_query("Update `valtis` set `viewed`=viewed+1 where `id`='$id'");
}
