...
function addCar($content, $model, $color, $year, $price, $way, $tp, $typeeng,$category ,$modif, $obem, $typekyzov, $kpp, $cash,$city, $name, $surname, $city2, $phone, $oldprice,$period,$diller,$foreign,$dilservice,$owners,$repair,$userid, $show, $garanty, $vin = "", $title = "") {
...
//	$period = 
	...
	sql_query("INSERT INTO `auto` (`date`,`content`, `name`, `tel`,`adress`,`city`,`model`,`color`,`year`,`price`,`way`,`tp`,`typeeng`,`category` ,`modif`,`obem`,`typekyzov`,`kpp`,`cash`, `show`, `surname`, `oldprice`, `enddate`, `diller`, `foreign`, `dilservice`,`owners`,`repair`, `userid`,`garanty`, `vin`, `title`) 
VALUES (NOW(), '$content','$name', '$phone','$city','$city2','$model','$color','$year','$price','$way','$tp','$typeeng','$category' ,'$modif','$obem','$typekyzov','$kpp','$cash', '$show','$surname','$oldprice', DATE_ADD(NOW(), INTERVAL $period), '$diller', '$foreign','$dilservice','$owners','$repair','$userid','$garanty', '$vin', '$title')");
	return mysql_insert_id();

}
function SaveCar($id,$userid,$content, $model, $color, $year, $price, $way, $tp, $typeeng,$category ,$modif, $obem, $typekyzov, $kpp, $cash,$city, $name, $surname, $city2, $phone, $oldprice,$period,$diller,$foreign,$dilservice,$owners,$repair,$userid, $show,$garanty, $vin, $title) {
...
function listAllAuto($page){
	...
	$res = sql_all("SELECT SQL_CALC_FOUND_ROWS `auto`.`prod`,`auto`.`id`,`cars`.`name` as `car_name`,`brands`.`name` as `brand_name`,`auto`.`topic`,`auto`.`cash`,`auto`.`price`,`auto`.`oldprice`,`auto`.`year`,`auto`.`tp`,`auto`.`show`,`auto`.`category`,`date`, `way`, `kpp`, `color`, `obem` 
...
function searchAllCar($brand, $fromprice, $toprice, $yearstart, $yearend, $engine, $page, $category){
$category = (int)$category;
	$page = (int)$page;
	if ($page < 1) {
		$page = 1;
...
function getPublication($url, $lang) {
	...
	return sql_all("SELECT * FROM `text_content` WHERE `url`='$url'&&`lang`='$lang'");
}
...
function getPagesList() {
	return sql_all("SELECT * FROM `text_content` `t` WHERE `t`.`url` != '404' AND `t`.`url` != 'main' AND `t`.`url` != 'auctions' AND `t`.`url` != 'contact' AND `t`.`url` != 'faq' ORDER BY `t`.`ves` ASC");
}
...
function addProduct($name, $price, $currency, $description, $bestoffers){
	$name = mysql_real_escape_string($name);
	$currency = mysql_real_escape_string($currency);
	$description = mysql_real_escape_string($description);
	$price = (int)$price;
	$bestoffers = (int)$bestoffers;
	sql_query("INSERT INTO `product` (`name`, `price`, `currency`, `description`, `date`, `bestoffers`) VALUES ('$name', $price, '$currency', '$description', now(),'$bestoffers')");
	return mysql_insert_id();
}
function editProduct($id, $name, $price, $currency, $description, $bestoffers){
	$name = mysql_real_escape_string($name);
	$currency = mysql_real_escape_string($currency);
	$description = mysql_real_escape_string($description);
	$price = (int)$price;
	$bestoffers = (int)$bestoffers;
	sql_query("UPDATE `product` SET `name`='$name', `price`=$price, `currency` = '$currency', `bestoffers` = '$bestoffers' WHERE `id`=$id");
}
...
function addMoto($uid, $mark, $model, $year, $color, $tip_dv, $obem, $prob, $tprob, $cost, $option, $oth_inf, $time, $pic1, $pic2, $pic3, $pic4, $pic5, $pic6, $pic7, $pic8, $pic9, $pic10, $title, $vin){
$uid = (int)$uid;
$year = (int)$year;
$prob = (int)$prob;
$cost = (int)$cost;
	...
	(`uid`, `mark`, `model`, `year`, `color`, `tip_dv`, `obem`, `prob`, `tprob`, `cost`, `option`, `oth_inf`, `time`, `pic1`, `pic2`, `pic3`, `pic4`, `pic5`, `pic6`, `pic7`, `pic8`, `pic9`, `pic10`, `title`, `vin`)
	values
	('$uid', '$mark', '$model', '$year', '$color', '$tip_dv', '$obem', '$prob', '$tprob', '$cost', '$option', '$oth_inf', '$time', '$pic1', '$pic2', '$pic3', '$pic4', '$pic5', '$pic6', '$pic7', '$pic8', '$pic9', '$pic10', '$title', '$vin')");
}


function getAllMotobyAdmin(){
...
function updMotoInf($id, $mark, $model, $year, $color, $tip_dv, $obem, $prob, $tprob, $cost, $option, $oth_inf, $title, $vin){
$id = (int)$id;
sql_query("UPDATE `moto` SET `title`='$title', `vin`='$vin', `mark`='$mark', `model`='$model', `year`='$year', `color`='$color', `tip_dv`='$tip_dv', `obem`='$obem', `prob`='$prob', `tprob`='$tprob', `oth_inf`='$oth_inf', `cost`='$cost', `option`='$option' WHERE `id`=$id");
}
function updMotoFoto($id, $pic1, $pic2, $pic3, $pic4, $pic5, $pic6, $pic7, $pic8, $pic9, $pic10){
$id = (int)$id;
sql_query("UPDATE `moto` SET `pic1`='$pic1', `pic2`='$pic2', `pic3`='$pic3', `pic4`='$pic4', `pic5`='$pic5', `pic6`='$pic6', `pic7`='$pic7', `pic8`='$pic8', `pic9`='$pic9', `pic10`='$pic10' WHERE `id`=$id");
}
function delFoto($id, $pic){
$id = (int)$id;
sql_query("UPDATE `moto` SET `$pic`='' WHERE `id`=$id");
}
function getAllMotobyBrand($id){
return sql_all("SELECT * FROM `moto` WHERE `mark`='$id' order by id");
}
function getAllMotoMain(){
return sql_all("SELECT 	* FROM `moto` order by id DESC limit 20 ");
}



function getAllBoatbyAdmin(){
...
function updBoatInf($id, $brand, $name, $tip_boat, $vin, $title, $year, $prod, $prosm, $cost, $time, $length, $width, $height, $places, $topl_potr, $dvig, $oth_inf, $option){
$id = (int)$id;
sql_query("UPDATE `boat` SET `brand`='$brand', `name`='$name', `tip_boat`='$tip_boat', `vin`='$vin', `title`='$title', `year`='$year', `prod`='$prod', `prosm`='$prosm', `cost`='$cost', `time`='$time', `length`='$length', `width`='$width', `height`='$height', `places`='$places', `topl_potr`='$topl_potr', `dvig`='$dvig', `oth_inf`='$oth_inf', `option`='$option' WHERE `id`=$id");
}
function updBoatFoto($id, $pic1, $pic2, $pic3, $pic4, $pic5, $pic6, $pic7, $pic8, $pic9, $pic10){
$id = (int)$id;
sql_query("UPDATE `boat` SET `pic1`='$pic1', `pic2`='$pic2', `pic3`='$pic3', `pic4`='$pic4', `pic5`='$pic5', `pic6`='$pic6', `pic7`='$pic7', `pic8`='$pic8', `pic9`='$pic9', `pic10`='$pic10' WHERE `id`=$id");
}
function delBoat($id, $pic){
$id = (int)$id;
sql_query("UPDATE `boat` SET `$pic`='' WHERE `id`=$id");
}
function getAllBoatbyBrand($id){
return sql_all("SELECT * FROM `boat` WHERE `brand`='$id' order by id");
}
function getAllBoatMain(){
return sql_all("SELECT 	* FROM `boat` order by id DESC limit 20 ");
}
...
function addBoat($uid, $brand, $name, $tip_boat, $vin, $title, $year, $prod, $prosm, $cost, $time, $length, $width, $height, $places, $topl_potr, $dvig, $oth_inf, $option, $pic1, $pic2, $pic3, $pic4, $pic5, $pic6, $pic7, $pic8, $pic9, $pic10){

	 sql_query("insert into `boat` 
	(`uid`, `brand`, `name`, `tip_boat`, `vin`, `title`, `year`, `prod`, `prosm`, `cost`, `time`, `length`, `width`, `height`, `places`, `topl_potr`, `dvig`, `oth_inf`, `option`, `pic1`, `pic2`, `pic3`, `pic4`, `pic5`, `pic6`, `pic7`, `pic8`, `pic9`, `pic10`)
	values
	('$uid', '$brand', '$name', '$tip_boat', '$vin', '$title', '$year', '$prod', '$prosm', '$cost', '$time', '$length', '$width', '$height', '$places', '$topl_potr', '$dvig', '$oth_inf', '$option', '$pic1', '$pic2', '$pic3', '$pic4', '$pic5', '$pic6', '$pic7', '$pic8', '$pic9', '$pic10')");
}
