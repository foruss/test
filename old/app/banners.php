...
echo('<h1>Cписок банеров</h1>');
	...
	<tr><td height="22" style="padding: 0 0 0 5px"><a href="?psl=app&do=banners&step=show&pos=2"><b>2</b> позиция (580x90)</a></td></tr>
	...
	<tr><td height="22" style="padding: 0 0 0 5px"><a href="?psl=app&do=banners&step=show&pos=7"><b>7</b> позиция (190x90)</a></td></tr>
</table></div>');
...
if (!isset($_POST['submit'])) {
	...
	<tr>
		...
		<td height="22" align="center"><select onchange="cmat(this.value);" style="width: 360px" size="1" name="pos"><option value="">Выберите</option><option value="1">1 позиция (190x90)</option><option value="2">2 позиция (580x90)</option><option value="3">3 позиция (340x168)</option><option value="4">4 позиция (180x150)</option><option value="5">5 позиция (140x140)</option><option value="6">6 позиция (790x120)</option><option value="7">7 позиция (190x90)</option></select></td>
	</tr>	<tr>
...
$insa=mysql_query("Insert into `adv` set `pos`='$pos', `lang`='$lang', `on`='$on', `type`='$type', `reg`='$reg', `url`='$url', `source`='$script', `ban`='$baneris', `sdata`='$sdata', `edata`='$edata', `maxshow`='$shows', `maxclick`='$clicks', `width`='$mw', `height`='$mh', `ltype`='$limit_by'") or die(mysql_error());
...
echo('<table border="1" width="300" cellspacing="0" cellpadding="0" style="border-collapse: collapse; margin: 0 0 10px 0" bordercolor="#E1E1E1">
	...
	<tr>
		<td bgcolor="#F8F8F8" align="right"><div style="float: left; margin: 2px; font-size: 8pt"><a class="spc_m" href="?psl=app&do=banners&step=edit&id='.$id.'">Изменить</a> | <a class="spc_m" onClick="if(confirm(\'Вы уверены, что хотите удалить это объявление?\')); else return false;" href="?psl=app&do=banners&step=delete&pos='.$pos.'&id='.$id.'&lang='.$lang.'">Удалить</a></div><div style="float: right; margin: 2px;">'.$status.'</div></td>
	</tr>
</table>');
}
echo('</div>');
...
$ask=mysql_num_rows($get_info);
	...
	<tr>
		<td height="22" style="padding: 0 2px 0 2px" width="120">Позиция:</td>
		<td height="22" align="center"><select onchange="cmat(this.value);" style="width: 360px" size="1" name="pos"><option value="">Выберите</option><option '); if ($pos == '1') { echo " selected "; } else { } echo(' value="1">1 позиция (190x90)</option><option '); if ($pos == '2') { echo " selected "; } else { } echo(' value="2">2 позиция (580x90)</option><option '); if ($pos == '3') { echo " selected "; } else { } echo(' value="3">3 позиция (340x168)</option><option '); if ($pos == '4') { echo " selected "; } else { } echo(' value="4">4 позиция (180x150)</option><option '); if ($pos == '5') { echo " selected "; } else { } echo(' value="5">5 позиция (140x140)</option><option '); if ($pos == '6') { echo " selected "; } else { } echo(' value="6">6 позиция (790x120)</option><option '); if ($pos == '7') { echo " selected "; } else { } echo(' value="7">7 позиция (190x90)</option></select></td>
	</tr>	<tr>
		<td height="22" style="padding: 0 2px 0 2px" width="120">Язык:</td>
...
$id = $_GET['id'];
$insa=mysql_query("Update `adv` set `pos`='$pos', `lang`='$lang', `on`='$on', `type`='$type', `url`='$url', `source`='$script', ".$ban_src." `sdata`='$sdata', `edata`='$edata', `limit`='$shows', `climit`='$clicks', `width`='$mw', `height`='$mh', `ltype`='$limit_by' where `id`='$id'") or die(mysql_error());
echo('<script lang="javascript">window.location="?psl=app&do=banners&step=show&pos='.$pos.'&lang='.$lang.'"</script>');
}
...
} else if ($step == 'remove_ban') {
	$id = $_GET['id'];
	$get_if=mysql_query("Select * from `adv` where `id`='$id'");
	$omo = mysql_fetch_array($get_if);
	$src = $omo['ban'];
	@unlink("$src");
	$get_if=mysql_query("Update `adv` set `ban`='' where `id`='$id'");
	echo('<script lang="javascript">window.location="?psl=app&do=banners&step=edit&id='.$id.'"</script>');
...
$pos = $_GET['pos'];
$lang = $_GET['lang'];
$updaiz=mysql_query("Update `adv` set `on`='$w' where `id`='$id'");
echo('<script lang="javascript">window.location="?psl=app&do=banners&step=show&pos='.$pos.'&lang='.$lang.'"</script>');
}
?>