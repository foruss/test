<div class="u_block"><div class="u_block-cont">
					<?php 
					$get_markes=mysql_query("Select distinct(marke) from `auto` where `kateg`='$kateg' order by `marke` asc");
					while($mar=mysql_fetch_array($get_markes)) {
					$marke = $mar['marke'];
					$skk=mysql_num_rows(mysql_query("Select * from `auto` where `marke`='$marke' and `kateg`='$kateg'"));
					$get_minfo=mysql_query("Select * from `marke` where `id`='$marke'");
...
if (!empty($step)) {
$get_mark=mysql_query("Select * from `marke` where `url`='$step'");
$tt=mysql_fetch_array($get_mark);
$mid = $tt['id'];
$where = " where `marke`='$mid' and `kateg`='$kateg'";
$plus = $step.".html";
} else {
...
<div style="float: right; width: 169px"><div class="rrd">'.$kt.' '.$kaina.'</div>'.$box.'</div>
</div>');
