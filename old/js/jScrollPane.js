	...
	return this.each(
		function()
		{
			...
			if (percentInView < .99) {
				...
				var dragH = Math.max(Math.min(percentInView*(paneHeight-settings.arrowSize*2), settings.dragMaxHeight), settings.dragMinHeight)*0.9;
					...
					function(event)
					{
						...
						if (!eleInView) {
							...
							if (eleTop > viewportTop) { // element is below viewport - scroll so it is at bottom.
								destPos += $(this).height() + settings.scrollbarMargin - paneHeight;
							}
							scrollTo(destPos);
						}
					}
