...
function addProduct($name, $price, $currency, $description, $bestoffers, $for_free_text, $model, $sost, $kol){
	$name = mysql_real_escape_string($name);
	...
	sql_query("INSERT INTO `product` (`name`, `price`, `currency`, `description`, `date`, `bestoffers`, `for_free_text`, `model`, `sost`, `kol`) VALUES ('$name', $price, '$currency', '$description', now(),'$bestoffers','$for_free_text','$model','$sost','$kol')");
	return mysql_insert_id();
}
...
function addBaner($link, $show, $tip){
	$show = (int)$show;
	$tip = (int)$tip;
