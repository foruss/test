...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'product_city'     => $query->row['product_city'],
				'product_hwy'      => $query->row['product_hwy'],
				'ean'              => $query->row['ean'],
				'jan'              => $query->row['jan'],
				'isbn'             => $query->row['isbn'],
				'mpn'              => $query->row['mpn'],
	...
	public function getProductOptions($product_id) {
		...
		$product_newoption_query2 = $this->db->query("SELECT pn.product_id, pn.newoption_id, pn.value, n.name FROM 
		`" . DB_PREFIX . "product_newoption` pn,
		`" . DB_PREFIX . "newoption` n

		WHERE pn.product_id = '" . (int)$product_id . "'
		AND      pn.newoption_id        = n.newoption_id
		AND      pn.value        = 1");
	...
	public function getProductByManufacturer($manufacturer_id, $product_id) {
		$product_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.manufacturer_id = '" . (int)$manufacturer_id . "' AND p.product_id != '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
